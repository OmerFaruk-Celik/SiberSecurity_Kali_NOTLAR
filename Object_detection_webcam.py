...
def orta_nokta(scores,classes,boxes,image):
	for i,box in enumerate(boxes[0]):
		...
		if int((classes[0][i]) == 1 or classes[0][i]==2) and scores[0][i] > 0.7: 
		
			#box = tuple(boxes[i].tolist())  
			ymin, xmin, ymax, xmax = box#boxes[0][i]
			...
			cv2.putText(image, "X: "+str(xmid-320)+"  Y: "+str(-ymid+240),(xmid, ymid), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
			...
			koordinat_x = open("./atilay/koordinat_x", "w")
			koordinat_x.write(str(uzaklik_x))
			koordinat_x.close()
	
			koordinat_y = open("./atilay/koordinat_y", "w")
			koordinat_y.write(str(uzaklik_y))
			koordinat_y.close()	
